...
class WoWisePurchaseItemController extends Controller
{
    /**
     * @param $mrpIndent
     * @return string
    ...
    {
        $response = $mrpIndent->getData(['indentCreatedMenuId' => 5]);
    ...
    public function store(Request $request, MrpIndent $mrpIndent, WorkOrderItemAssembly $workOrderItemAssembly)
    {
        ...
        /*$woIds = collect($request->woIds);

        $woIds->each(function ($woId) {
            dd(WorkOrderItem::find($woId)->toArray());
        });
        dd('done');*/
