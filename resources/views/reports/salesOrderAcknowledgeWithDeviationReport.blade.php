...
<head>
    <meta charset="UTF-8">
    <title> SALES ORDER ACKNOWLEDGEMENT WITH DEVIATION</title>
</head>
<body>
<!-- Start Main Title -->
<table border="1px" cellpadding="5px">
    <tr>
        <td>
            <table>
                <tr>
                    <td style="text-align: center;" width="100%"><h3><b>SALES ORDER ACKNOWLEDGEMENT WITH DEVIATION</b>
                        </h3></td>
                </tr>
                <tr>
...
<table border="1px" cellpadding="5px">
    ...
    @if(!$request->filled('print_so_ack_without_contact'))
        <tr>
            <td width="50%">
                <table>
                    <tr>
...
<table border="1px" cellpadding="5px">
        ...
        <tr>
            ...
            <td width="{{$itemWidth}}%">
                <table>
                    ...
                    @endphp
                    @php
                        if($item->itemAddonDescription){
                        $itemAddDescription = nl2br($item->itemAddonDescription);
                        $itemAddDescriptions = explode('<br />', $itemAddDescription);
                            foreach ($itemAddDescriptions as $value){
                                echo "<tr><td width='100%'>$value</td></tr>";
                            }
                        }
                    @endphp
            ...
            @endif
            <td width="6%" style="text-align: center;">{{ $item->uom?->uom }}</td>
    ...
    <!-- End Term & Condition -->
    @if(isset($salesOrder->deviation) and !empty($salesOrder->deviation))
        <!-- Start Deviation -->
        <tr>
            <td><b>The above order needs certain corrections as indicated below.</b>@php echo str_repeat('<br>',2); @endphp<b>{{ $salesOrder->deviation }}</b></td>
        </tr>
        <!-- End Deviation -->
    @endif
    ...
    <tr>
        <td width="50%">
            <table>
                <tr>
                    <td width="25%">GST IN No.</td>
                    <td width="75%">: {{ $salesOrder->companyDetails?->gstNumber }}</td>
                </tr>
                <tr>
                    <td width="25%">PAN No.</td>
                    <td width="75%">: {{ $salesOrder->companyDetails?->panNumber }}</td>
                </tr>
                <tr>
                    <td width="25%">ARN No.</td>
                    <td width="75%">: AA2412160039869</td>
                </tr>
                <tr>
                    <td width="25%">IEC No.</td>
                    <td width="75%">: 3409005072</td>
                </tr>
            </table>
        </td>
        <td width="50%">
