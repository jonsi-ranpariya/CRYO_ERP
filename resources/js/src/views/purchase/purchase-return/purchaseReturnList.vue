                ...
                <span v-if="props.column.field === 'action'">
                      <span>
                        ...
                        >
                          ...
                          <b-dropdown-item :to="{ name: 'edit-purchase-return', params: { id: props.row._id} }">
                            <feather-icon
                                icon="Edit2Icon"
                                class="mr-50"
                            />
                          ...
                          <b-dropdown-item @click="deletePurchaseReturn(props.row._id)">
                            <feather-icon
                                icon="TrashIcon"
                                class="mr-50"
                            />
    ...
    export default {
        ...
        setup() {
            ...
            const columns = ref([
                {
                    label: 'Sell-To Party',
                    field: 'extra_data.sellToParty.partyName',
                },
                {
                    label: 'Ship-To Party',
                    field: 'extra_data.shipToParty.partyName',
                },
                {
                    label: 'Invoice No',
                    field: 'prInvoiceNo',
                },
                {
                    label: 'Invoice',
                    field: 'prInvoice',
                    formatFn: val => getInvoiceName(val)
                },
                {
                    label: 'Invoice Date',
                    field: 'prInvoiceDate',
                ...
                {
                    label: 'Invoice Type',
                    field: 'extra_data.invoiceType.description',
                },
                {
                    label: 'Purchase Bill No',
                    field: 'prPurchaseBillNo',
                },
                {
                    label: 'Purchase Bill Date',
                    field: 'prPurchaseDate',
                },
                {
                    label: 'Action',
                    field: 'action'
            ...
            const refetchData = async () => {
                ...
                await axios.get('/api/new/purchase-return').then(res => {
            ...
            const getInvoiceName = (statusValue) => {
                let valObj = InvoiceOptions.value.filter(function (elem) {
           ...
           const deletePurchaseReturn = async (id) => {
               await Swal.fire({
                   ...
                   if (result.value) {
                       store.dispatch('purchase-store-module/deletePurchaseReturn', {id: id}).then(res => {
                           if (res.status === 200) {
                               toast({
                                   ...
                                   props: {
                                       ...
                                       variant: 'success',
                                       text: 'You have successfully Deleted Purchase Return.',
                                   },
                               })
                           } else {
                               toast({
