<template>
    ...
    >
        <div>
            <grade-master-add-edit
                :gradeFormData.sync="formData"
            ...
            <b-card-code>
                ...
                <vue-good-table
                    ...
                    :rows="$store.getters['hrMasterStoreModule/getGradeMasterList']"
                    :line-numbers="true"
                    :rtl="direction"
                    :search-options="{
                        enabled: true,
                    ...
                    >
                        ...
                        <span v-if="props.column.field === 'action'"><span>
                            ...
                            >
                              ...
                              <b-dropdown-item @click="editGradeMaster(props.row)">
                                <feather-icon
                                    icon="Edit2Icon"
                                    class="mr-50"
                                />
                              ...
                              <b-dropdown-item @click="deleteGradeMasterData(props.row._id)">
                                <feather-icon
                                    icon="TrashIcon"
                                    class="mr-50"
                                />
...
<script>
    ...
    import gradeMasterAddEdit from "./gradeMasterAddEdit"
    ...
    export default {
        ...
        setup() {
            ...
            store.dispatch('hrMasterStoreModule/listGradeMaster');
            ...
            const addEditGradeMaster = async () => {
                await store.commit('hrMasterStoreModule/sideBarShowHide', true);
            }

            const editGradeMaster = async (data) => {
                formData.value = data;
                await store.commit('hrMasterStoreModule/sideBarShowHide', true);
            }
            const deleteGradeMasterData = async (id) => {
                Swal.fire({
                    title: 'Are you sure?',
                    text: "Delete Grade Master",
                    icon: 'warning',
                    ...
                    if (result.value) {
                        store.dispatch('hrMasterStoreModule/deleteGradeMaster', {id: id})
                            .then(res => {
                                if (res.status === 200) {
                                    toast({
                                        ...
                                        position: 'top-right',
                                        props: {
                                            title: 'Grade Master Deleted',
                                            icon: 'CheckSquareIcon',
                                            variant: 'success',
                                            text: 'You have successfully Deleted Grade Master',
                                        },
                                    })
                                }
                            }).catch(() => {
