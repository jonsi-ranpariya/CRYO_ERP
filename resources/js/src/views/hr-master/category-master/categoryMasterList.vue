<template>
    ...
    >
        <div>
            <category-master-add-edit
                :categoryFormData.sync="formData"
            ...
            <b-card-code>
                ...
                <vue-good-table
                    ...
                    :rows="$store.getters['hrMasterStoreModule/getCategoryMasterList']"
                    :line-numbers="true"
                    :rtl="direction"
                    :search-options="{
                        enabled: true,
                    ...
                    >
                        ...
                        <span v-if="props.column.field === 'action'"><span>
                            ...
                            >
                              ...
                              <b-dropdown-item @click="editCategoryMaster(props.row)">
                                <feather-icon
                                    icon="Edit2Icon"
                                    class="mr-50"
                                />
                              ...
                              <b-dropdown-item @click="deleteCategoryMasterData(props.row._id)">
                                <feather-icon
                                    icon="TrashIcon"
                                    class="mr-50"
                                />
...
<script>
    ...
    import categoryMasterAddEdit from './categoryMasterAddEdit'
    ...
    export default {
        ...
        setup() {
            ...
            store.dispatch('hrMasterStoreModule/listCategoryMaster');
            ...
            const addEditCategoryMaster = async () => {
                await store.commit('hrMasterStoreModule/sideBarShowHide', true);
            }

            const editCategoryMaster = async (data) => {
                formData.value = data;
                await store.commit('hrMasterStoreModule/sideBarShowHide', true);
            }
            const deleteCategoryMasterData = async (id) => {
                Swal.fire({
                    title: 'Are you sure?',
                    text: "Delete Category Master",
                    icon: 'warning',
                    ...
                    if (result.value) {
                        store.dispatch('hrMasterStoreModule/deleteCategoryMaster', {id: id})
                            .then(res => {
                                if (res.status === 200) {
                                    toast({
                                        ...
                                        position: 'top-right',
                                        props: {
                                            title: 'Category Master Deleted',
                                            icon: 'CheckSquareIcon',
                                            variant: 'success',
                                            text: 'You have successfully Deleted Category Master',
                                        },
                                    })
                                }
                            }).catch(() => {
