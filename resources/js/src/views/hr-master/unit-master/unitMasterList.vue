<template>
    ...
    >
        <div>
            <unit-master-add-edit
                :unitFormData.sync="formData"
            ...
            <b-card-code>
                ...
                <vue-good-table
                    ...
                    :rows="$store.getters['hrMasterStoreModule/getUnitMasterList']"
                    :line-numbers="true"
                    :rtl="direction"
                    :search-options="{
                        enabled: true,
                    ...
                    >
                        ...
                        <span v-if="props.column.field === 'action'"><span>
                            ...
                            >
                              ...
                              <b-dropdown-item @click="editUnitMaster(props.row)">
                                <feather-icon
                                    icon="Edit2Icon"
                                    class="mr-50"
                                />
                              ...
                              <b-dropdown-item @click="deleteUnitMasterData(props.row._id)">
                                <feather-icon
                                    icon="TrashIcon"
                                    class="mr-50"
                                />
...
<script>
    ...
    import unitMasterAddEdit from './unitMasterAddEdit';
    ...
    export default {
        ...
        setup() {
            ...
            store.dispatch('hrMasterStoreModule/listUnitMaster');
            ...
            const addEditUnitMaster = async () => {
                await store.commit('hrMasterStoreModule/sideBarShowHide', true);
            }

            const editUnitMaster = async (data) => {
                formData.value = data;
                await store.commit('hrMasterStoreModule/sideBarShowHide', true);
            }
            const deleteUnitMasterData = async (id) => {
                Swal.fire({
                    title: 'Are you sure?',
                    text: "Delete Unit Master",
                    icon: 'warning',
                    ...
                    if (result.value) {
                        store.dispatch('hrMasterStoreModule/deleteUnitMaster', {id: id})
                            .then(res => {
                                if (res.status === 200) {
                                    toast({
                                        ...
                                        position: 'top-right',
                                        props: {
                                            title: 'Unit Master Deleted',
                                            icon: 'CheckSquareIcon',
                                            variant: 'success',
                                            text: 'You have successfully Deleted Unit Master',
                                        },
                                    })
                                }
                            }).catch(() => {
