    ...
    <b-card-code>
        <div>
            <div class="text-right">
                <b-button variant="outline-primary" v-b-modal.modal-primary>Add Cost Center</b-button>
            </div>
            <hr>
            <TreeNode v-for="node in nodes" :key="node.id" :node="node" :itemData="formData"/>
            <add-cost-center-group-modal :itemId="''"/>
        </div>
...
<script>
    ...
    export default {
        ...
        computed: {
            nodes() {
                return this.$store.getters["financeMasterStoreModule/getCostCenterList"]
            }
        }
    }
</script>
