    ...
    export default {
        ...
        setup({purchaseOrderModal}, {jobWorkModal}) {
            ...
            const refetchData = async () => {
                ...
                await axios.get(`/api/new/gate-inward-receipt/${router.currentRoute.params.id}`).then((res) => {
            ...
            const saveFormData = async () => {
                await store.dispatch('inventory-store-module/updateGateInwardReceipt', formData.value).then(res => {
                    if (res.status === 200) {
                        toast({
                            component: ToastificationContent,
                            position: 'top-right',
                            props: {
                                title: 'Gate Inward Receipt Updated',
                                icon: 'CheckSquareIcon',
                                variant: 'success',
                                text: 'You have successfully Updated Gate Inward Receipt',
                            },
                        })
                    } else {
                        toast({
                    ...
                    router.push({name: 'gate-inward-receipt'})
                })
            }
