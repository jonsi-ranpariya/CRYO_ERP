    ...
    export default {
        ...
        setup() {
            ...
            const transactionOption = ref([
                {label: 'Opening Stock', value: 1},
                {label: 'Receipt', value: 2},
                {label: 'Issue', value: 3},
                {label: 'Return', value: 4},
                {label: 'Rejection(Shop)', value: 5},
            ])
            ...
            const resetFormData = () => {
                formData.value = JSON.parse(JSON.stringify(blackFormData))
            }
            const refetchData = async () => {
                showOverlayLoading.value = true
                await axios.get(`/api/new/transaction-inventory/${router.currentRoute.params.id}`).then(res => {
            ...
            const saveFormData = async () => {
                await store.dispatch('inventory-store-module/updateTransaction', formData.value).then(res => {
                    if(res.status === 200){
                        toast({
                            ...
                            props: {
                                ...
                                variant: 'success',
                                text: `You have successfully Update Transaction.`,
                            },
                        })
                    } else {
                        toast({
