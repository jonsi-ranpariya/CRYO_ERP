                        ...
                        <b-col md="12">
                            ...
                            >
                                ...
                                <v-select
                                    ...
                                    :clearable="false"
                                    input-id="TransMode"
                                    v-model="formData.transporterPartyId"
...
export default {
    ...
    setup(props) {
        ...
        axios.get('/api/party/get-master-transporter-options')
            .then((res) => {
                transportOptions.value = res.data.data
            })

        const getPartyDetails = async (id) => {
            await axios.get(`/api/party/edit/${id}`)
                .then((res) => {
                    formData.value.transporter = res.data.data.partyName
                    formData.value.transporterId = res.data.data.taxDetails.gstin ?? null
                })
        }
        ...
        const saveFormData = async () => {
            ...
            await store.dispatch('dispatch-store-module/updateNewInvoiceChallan', submitData)
                .then((res) => {
                    if (res.status === 200) {
                        toast({
        ...
        setTimeout(() => {
            if (!formData.value.timeIssueInvoice) {
                formData.value.timeIssueInvoice = _(new Date(), 'hh:mm:ss A')
                    .format('HH:mm:ss')
            }
            if (!formData.value.timeRemovalGood) {
                formData.value.timeRemovalGood = _(new Date(), 'hh:mm:ss A')
                    .format('HH:mm:ss')
            }
