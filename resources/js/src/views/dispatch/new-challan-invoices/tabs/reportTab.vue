<template>
    <div>
        ...
        <b-row>
            <b-col md="12">
                <b-row>
                    ...
                    <b-col md="3">
                        <div
                            v-if="currentInvoiceData.master_gst_details && Object.keys(currentInvoiceData.master_gst_details).length !== 0"
                        >
                            <b-button
                                type="button"
                                class="ml-1"
                                variant="primary"
                                @click="cancelIrn"
                                v-if="currentInvoiceData.master_gst_details.Irn"
                            >
                                Cancel E-INVOICE
                            </b-button>
                        </div>
        ...
        >
                    ...
                    >
                        ...
                        <template #cell(action)="data">
                            ...
                            >
                                ...
                                <b-dropdown-item @click="editStickerItem(data.index)">
                                    <feather-icon
                                        icon="Edit2Icon"
                                        class="mr-50"
                                    />
                                ...
                                <b-dropdown-item @click="deleteStickerItem(data.index)">
                                    <feather-icon
                                        icon="TrashIcon"
                                        class="mr-50"
                                    />
            ...
            <div v-if="tableItemData.length > 0">
                <b-row>
                    <b-col md="6">
                        <b-form-group
                            label="Item Code"
                        ...
                        >
                            <b-form-input
                                disabled
                                id="itemCode"
                                v-model="tableItemData[updatedKey].itemCode"
                            />
                        </b-form-group>
                        <b-form-group
                        ...
                        >
                            <b-form-input
                                disabled
                                id="itemDescription"
                                v-model="tableItemData[updatedKey].itemDescription"
                            />
                        </b-form-group>
                        <b-form-group
                        ...
                        >
                            <b-form-input
                                ...
                                v-model="tableItemData[updatedKey].itemDetailDescription"
                            />
                        </b-form-group>
                        <b-form-group
                        ...
                        >
                            <b-form-input
                                disabled
                                id="drawingNumber"
                                v-model="tableItemData[updatedKey].itemDrawingNo"
                            />
                        </b-form-group>
                    </b-col>
                    <b-col md="6">
                        ...
                        >
                            <b-form-input
                                disabled
                                id="drawingRevision"
                                v-model="tableItemData[updatedKey].itemRevision"
                            />
                        </b-form-group>
                        <b-form-group
                            label="Uom"
                        ...
                        >
                            <b-form-input
                                disabled
                                id="uom"
                                v-model="tableItemData[updatedKey].uom_details.uom"
                            />
                        </b-form-group>
                        <b-form-group
                            label="Quantity"
                        ...
                        >
                            <b-form-input
                                id="itemQuantity"
                                v-model="tableItemData[updatedKey].itemQuantity"
                            />
                        </b-form-group>
                    </b-col>
                    <b-col md="12">
...
<script>
    ...
    import {MASTER_GST_EMAIL} from "@core/utils/masterGstHelper";
    ...
    export default {
        ...
        setup({cancelIrnModal, stickerPrinting}) {
            const currentInvoiceData = ref({})
            ...
            axios.get(`/api/new-challan-and-invoice/${router.currentRoute.params.id}`).then(res => {
                currentInvoiceData.value = res.data.data
            });
            ...
            const openPdf = async () => {
                ...
                await axios.post('/pdf/generate-pdf', {
                    ...
                    newChallanInvoiceId: router.currentRoute.params.id,
            ...
            const generateIrn = async () => {
                await Swal.fire({
                    ...
                    if (result.value) {
                        axios.get('/api/master-gst/get-items-and-final-amounts/'+router.currentRoute.params.id ).then((res) => {
            ...
            const generateEInvoiceDetails = async (itemData) => {
                ...
                let data = {
                    ...
                    "TranDtls": {
                        "TaxSch": "GST",
                        "SupTyp": currentInvoiceData.value.reportSupplyType,
                        "RegRev": "N",
                        "EcmGstin": null,
                        "IgstOnIntra": "N"
                    },
                    "DocDtls": {
                        "Typ": "INV",
                        "No": currentInvoiceData.value.invoiceNo,
                        "Dt": currentInvoiceData.value.invoiceDate
                    },
                    "SellerDtls": {
                        "Gstin": currentInvoiceData.value.company_details.ewayGstin,
                        "LglNm": currentInvoiceData.value.company_details.companyName,
                        "TrdNm": currentInvoiceData.value.company_details.companyName,
                        "Addr1": currentInvoiceData.value.company_details.address.substring(0, 100),
                        "Loc": currentInvoiceData.value.company_details.city_data.city.toUpperCase(),
                        "Pin": parseInt(currentInvoiceData.value.company_details.pinCode),
                        "Stcd": currentInvoiceData.value.company_details.state_data.stateCode,
                    },
                    "BuyerDtls": {
                        "Gstin": currentInvoiceData.value.buyerDetails.buyerGstinNo,
                        "LglNm": currentInvoiceData.value.buyerDetails.buyerName,
                        "TrdNm": currentInvoiceData.value.buyerDetails.buyerName,
                        "Pos": currentInvoiceData.value.buyer_extra_data.state.stateCode,
                        "Addr1": currentInvoiceData.value.buyerDetails.buyerAddress.substring(0, 100),
                        "Loc": currentInvoiceData.value.buyer_extra_data.city.city.toUpperCase(),
                        "Pin": parseInt(currentInvoiceData.value.buyerDetails.buyerPinCode),
                        "Stcd": currentInvoiceData.value.buyer_extra_data.state.stateCode,
                    },
                    "DispDtls": {
                        "Nm": currentInvoiceData.value.company_details.companyName,
                        "Addr1": currentInvoiceData.value.company_details.address.substring(0, 100),
                        "Loc": currentInvoiceData.value.company_details.city_data.city.toUpperCase(),
                        "Pin": parseInt(currentInvoiceData.value.company_details.pinCode),
                        "Stcd": currentInvoiceData.value.company_details.state_data.stateCode
                ...
                console.log(data)
                await axios.get(MASTER_API_BASE_URL + '/einvoice/authenticate?email=' + currentInvoiceData.value.company_details.emailId, {
                    headers: {
                        username: currentInvoiceData.value.company_details.ewayUsername,
                        password: currentInvoiceData.value.company_details.ewayPassword,
                        ip_address: MASTER_GST_IP_ADDRESS,
                        client_id: currentInvoiceData.value.company_details.ewayClientId,
                        client_secret: currentInvoiceData.value.company_details.ewayClientSecret,
                        gstin: currentInvoiceData.value.company_details.ewayGstin
                    }
                }).then(res => {
                ...
                await axios.post(MASTER_API_BASE_URL + '/einvoice/type/GENERATE/version/V1_03?email=' + currentInvoiceData.value.company_details.emailId, data, {
                    headers: {
                        ip_address: MASTER_GST_IP_ADDRESS,
                        client_id: currentInvoiceData.value.company_details.ewayClientId,
                        client_secret: currentInvoiceData.value.company_details.ewayClientSecret,
            ...
            const cancelIrn = async () => {
                await axios.get(`/api/new-challan-and-invoice/${router.currentRoute.params.id}`).then(res => {
                    cancelFormData.value.irnNumber = res.data.data.master_gst_details.Irn ?? null
                });
            ...
            const generateCancelIrnRequest = async () => {
                ...
                await axios.get(MASTER_API_BASE_URL + '/einvoice/authenticate?email=' + currentInvoiceData.value.company_details.ewayEmail, {
                    headers: {
                        username: currentInvoiceData.value.company_details.ewayUsername,
                        password: currentInvoiceData.value.company_details.ewayPassword,
                        ip_address: MASTER_GST_IP_ADDRESS,
            ...
            const generateEwayBillDetails = async () => {
                let authToken, data = {
                    "Irn": currentInvoiceData.value.master_gst_details.Irn,
                    "Distance": parseInt(currentInvoiceData.value.transportExportData.distance),
                    "TransMode": currentInvoiceData.value.transportExportData.transportMode ?? null,
                    "TransId": currentInvoiceData.value.transportExportData.transporterId ?? null,
                    "TransName": currentInvoiceData.value.transportExportData.transporter ?? null,
                    "TransDocNo": currentInvoiceData.value.transportExportData.lrNumber,
                    "TransDocDt": currentInvoiceData.value.transportExportData.lrDate,
                    "VehNo": currentInvoiceData.value.transportExportData.vehicalNo ?? null,
                    "VehType": currentInvoiceData.value.transportExportData.vehicleType ?? null
                };
                ...
                await axios.post(MASTER_API_BASE_URL + '​/einvoice/type/GENERATE_EWAYBILL/version/V1_03?email=' + MASTER_GST_EMAIL, data, {
                    headers: {
                        ip_address: MASTER_GST_IP_ADDRESS,
                        client_id: currentInvoiceData.value.company_details.ewayClientId,
                        client_secret: currentInvoiceData.value.company_details.ewayClientSecret,
            ...
            const ewayBillSiteRedirect = async () => {
                //window.open('https://ewaybillgst.gov.in/BillGeneration/EBPrint.aspx?cal=1', '_blank');
                window.open('https://einvoice1.gst.gov.in/', '_blank');
            }

            const tableItemField = ref([
                {key: 'index', label: 'Sr.'},
                {key: 'itemCode', label: 'Item Code'},
                {key: 'itemDescription', label: 'Item Desc.'},
                {key: 'uom_details.uom', label: 'uom'},
                {key: 'itemQuantity', label: 'Packing Qty.'},
                {key: 'action', label: 'Actions'},
            ])
            const tableItemData = ref([])
            const updatedKey = ref(0)

            const stickerPrintingModal = async () => {
                await axios.get(`/api/new-challan-and-invoice/${router.currentRoute.params.id}`).then(res => {
                    tableItemData.value = res.data.data.items_detail ?? []
                });
                stickerPrinting.value.show();
            }

            const stickerPrintingPreview = async () => {
                let printData = {
                    challanInvoiceId: router.currentRoute.params.id,
                    printItems: tableItemData.value
                }

                await axios.post('/pdf/sticker-print-preview', printData)
                    .then((res) => {
                        if (res.status === 200) {
            ...
            const editStickerItem = (key) => {
                updatedKey.value = key
            }

            const deleteStickerItem = (key) => {
                tableItemData.value.splice(key, 1)
                updatedKey.value = 0
            }
            ...
            return {
                ...
                resetCancelFormData,
                generateEWayBill,
                stickerPrintingModal,
                editStickerItem,
                deleteStickerItem,
                stickerPrintingPreview,
