...
export default {
    ...
    setup(props, {
        ...
        const formData = computed(() => props.bomFormData)
        ...
        const getItemDetails = async (id) => {
            await axios.get(`/api/item-master/${id}`)
                .then(res => {
                    ...
                    formData.value.assemblyFromVersionId = null
                })
        }

        const submitFormData = async () => {
            let itemVersionId = router.currentRoute.params.versionItemId ?? null
            if (formData.value.id === null) {
                formData.value.versionItemId = itemVersionId
                ...
                await emit('refetchData', { versionItemId: itemVersionId })
                await emit('resetFormData', true)
            }
        }
