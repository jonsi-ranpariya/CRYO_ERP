    ...
    <b-card-code>
        <b-row class="mb-1">
            <b-col cols="12" md="6" class="d-flex justify-content-start align-items-center">
                <b-button
                    variant="primary"
                    :to="{ name: 'add-job-work-rework-challan'}"
                > Add Job ReWork Challan
                </b-button>
            </b-col>
                ...
                <span v-if="props.column.field === 'action'">
                      <span>
                        ...
                        >
                          ...
                          <b-dropdown-item  :to="{ name: 'edit-job-work-rework-challan', params: { id: props.row._id} }">
                            <feather-icon
                                icon="Edit2Icon"
                                class="mr-50"
                            />
                          ...
                          <b-dropdown-item @click="deleteJobReWorkChallan(props.row._id)">
                            <feather-icon
                                icon="TrashIcon"
                                class="mr-50"
                            />
    ...
    export default {
        ...
        setup() {
            ...
            const columns = ref([
                {
                    ...
                    field: 'reWorkChallanNo',
                },
                {
                    label: 'ReWork Challan Date',
                    field: 'reWorkChallanDate',
                },
                {
            ...
            const refetchData = async () => {
                ...
                await axios.get('/api/new/job-work-rework-challan').then(res => {
            ...
            const deleteJobReWorkChallan = async (id) => {
                await Swal.fire({
                    title: 'Are you sure?',
                    text: "Delete Job ReWork Challan",
                    icon: 'warning',
                    ...
                    if (result.value) {
                        store.dispatch('job-rework-challan-store-module/deleteJobReWorkChallan', {id: id}).then(res => {
                            if (res.status === 200) {
                                toast({
                                    component: Toastification,
                                    position: 'top-right',
                                    props: {
                                        title: 'Job ReWork Challan Deleted',
                                        icon: 'CheckSquareIcon',
                                        variant: 'success',
                                        text: 'You have successfully Deleted Job ReWork Challan',
                                    },
                                })
                            } else {
                                toast({
