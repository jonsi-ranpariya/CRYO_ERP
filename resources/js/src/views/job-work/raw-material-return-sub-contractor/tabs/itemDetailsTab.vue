                        ...
                        <b-row>
                            <b-col md="6">
                                <b-form-group
                                    ...
                                    label-for="itemAcceptQty"
                                >
                                    <b-form-input
                                        v-model="formData.itemAcceptQty"
                                        id="itemAcceptQty"
                                    />
                                </b-form-group>
                            </b-col>
                            <b-col md="6">
                                <b-form-group
                                    label="Reject Quntity"
                                    label-for="itemRejectQty"
                                >
                                    <b-form-input
                                        v-model="formData.itemRejectQty"
                                        id="itemRejectQty"
                                    />
                                </b-form-group>
                            </b-col>
...
<script>
    ...
    export default {
        ...
        setup({itemMasterModal}) {
            ...
            const ITEMADDON_APP_STORE_MODULE_NAME = 'raw-material-item-store-module'
            ...
            axios.get('/api/get-item-job-order-option').then(res => {
            ...
            const blankFormData = {
                _id: null,
                rawId: router.currentRoute.params.id,
                itemId:null,
            ...
            const getItemMasterFilter = async () => {
                ...
                await axios.post('/api/new/get-issue-job-challan-item-filter', searchData.value).then((res) => {
            ...
            const resetModalData = () => {
                ...
                searchData.value.itemDrawingNumber = null
            ...
            const saveItemMasterDetail = async () => {
                await axios.post('/api/new/add-raw-material-sub-contractor-item', {
            ...
            const refetchData = async () => {
                isBusy.value = true
                await axios.get(`/api/new/get-raw-material-contractor-item/${router.currentRoute.params.id}`).then(res => {
            ...
            const deleteItemDetails = async (id) => {
                await Swal.fire({
                    ...
                    if (result.value) {
                        store.dispatch('raw-material-item-store-module/deleteRawMaterialItemDetail', {id: id}).then((res) => {
                            if (res.status === 200) {
                                toast({
                                    component: ToastificationContent,
                                    position: 'top-right',
                                    props: {
                                        title: 'Raw Material Item Deleted',
                                        icon: 'CheckSquareIcon',
                                        variant: 'success',
                                        text: 'You have successfully Deleted Raw MAterial Item',
                                    },
                                })
                            } else {
                                toast({
            ...
            const saveFormData = async () => {
                if (formData.value._id === null) {
                    store.dispatch('raw-material-item-store-module/addRawMaterialItemDetail', formData.value).then((res) => {
                        if (res.status === 200) {
                            toast({
                                component: ToastificationContent,
                                position: 'top-right',
                                props: {
                                    title: 'Raw Material Item Added',
                                    icon: 'CheckSquareIcon',
                                    variant: 'success',
                                    text: 'You have successfully Added Raw Material Item',
                                },
                            })
                        } else {
                            toast({
                    ...
                    store.dispatch('raw-material-item-store-module/updateRawMaterialItemDetail', formData.value).then((res) => {
                        if (res.status === 200) {
                            toast({
                                component: ToastificationContent,
                                position: 'top-right',
                                props: {
                                    title: 'Raw Material Item Updated',
                                    icon: 'CheckSquareIcon',
                                    variant: 'success',
                                    text: 'You have successfully Updated Raw Material Item',
                                },
                            })
                        } else {
                            toast({
