    ...
    <b-card-code>
        <b-row class="mb-1">
            <b-col cols="12" md="6" class="d-flex justify-content-start align-items-center">
                <b-button
                    variant="primary"
                    :to="{ name: 'add-scrap-receipt-entry'}"
                > Add Scrap Receipt Entry
                </b-button>
            </b-col>
                ...
                <span v-if="props.column.field === 'action'">
                      <span>
                        ...
                        >
                          ...
                          <b-dropdown-item :to="{ name: 'edit-scrap-receipt-entry', params: { id: props.row._id} }">
                            <feather-icon
                                icon="Edit2Icon"
                                class="mr-50"
                            />
                          ...
                          <b-dropdown-item @click="deleteScrapEntry(props.row._id)">
                            <feather-icon
                                icon="TrashIcon"
                                class="mr-50"
                            />
    ...
    export default {
        ...
        setup() {
            ...
            const refetchData = async () => {
                ...
                await axios.get('/api/new/scrap-receipt-entrys').then(res => {
            ...
            const deleteScrapEntry = async (id) => {
                await Swal.fire({
                    title: 'Are you sure?',
                    text: "Delete Scrap Entry",
                    icon: 'warning',
                    ...
                    if (result.value) {
                        store.dispatch('scrap-entry-store-module/deleteScrapReceiptEntry', {id: id}).then(res => {
                            if (res.status === 200) {
                                toast({
                                    component: Toastification,
                                    position: 'top-right',
                                    props: {
                                        title: ' Scrap Receipt Entry Deleted',
                                        icon: 'CheckSquareIcon',
                                        variant: 'success',
                                        text: 'You have successfully Deleted Scrap Receipt Entry',
                                    },
                                })
                            } else {
                                toast({
