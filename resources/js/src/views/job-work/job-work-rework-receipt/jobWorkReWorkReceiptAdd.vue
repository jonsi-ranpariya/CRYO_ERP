...
<script>
    ...
    export default {
        ...
        setup({itemMasterModal}){
            ...
            const ITEMADDON_APP_STORE_MODULE_NAME = 'job-rework-receipt-store-module'
            ...
            const openModal = async () => {
                await axios.get('/api/new/job-work-rework-challan-item').then(res => {
            ...
            const getItemMasterDetails = async (data) => {
                ...
                formData.value.jobReWorkId = data.jobReWorkId
                formData.value.jobReWorkItemId = data._id
                formData.value.jobWorkId = data.jobWorkId
                ...
                formData.value.itemDrawingNo = data.itemDrawingNumber
                formData.value.itemRevision = data.itemDrawingRevision
                formData.value.itemUom = data.itemUom
                formData.value.jobQty = data.itemQuntity
                formData.value.pendingJobQty = data.itemPendingQty
            ...
            const saveFormData = async () => {
                await store.dispatch('job-rework-receipt-store-module/addJobReWorkReceipt', formData.value).then(res => {
                    if (res.status === 200) {
                        toast({
                            component: ToastificationContent,
                            position: 'top-right',
                            props: {
                                title: 'Job ReWork Receipt Added',
                                icon: 'CheckSquareIcon',
                                variant: 'success',
                                text: `You have successfully Added Job ReWork Receipt.`,
                            },
                        })
                    } else {
                        toast({
                ...
                router.push({
                    name: 'edit-job-work-rework-receipt',  params: {
