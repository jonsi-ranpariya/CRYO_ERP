...
<script>
    ...
    export default {
        ...
        setup() {
            ...
            axios.get(`/api/new/job-work-reawork-receipt/${router.currentRoute.params.id}`).then(res => {
            ...
            const saveFormData = async () => {
                await store.dispatch('job-rework-receipt-store-module/updateJobReWorkReceipt', formData.value).then(res => {
                    if (res.status === 200) {
                        toast({
                            component: ToastificationContent,
                            position: 'top-right',
                            props: {
                                title: 'Job ReWork Receipt Updated',
                                icon: 'CheckSquareIcon',
                                variant: 'success',
                                text: `You have successfully Updated Job ReWork Receipt.`,
                            },
                        })
                    } else {
                        toast({
                    ...
                    router.push({name: 'job-work-rework-receipt'})
                })
            }
