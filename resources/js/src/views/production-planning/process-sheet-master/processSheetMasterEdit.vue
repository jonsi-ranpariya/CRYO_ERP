                            ...
                            <b-row>
                                ...
                                <b-col md="3">
                                    ...
                                    >
                                        <b-form-input
                                            id="processSheetNo"
                                            v-model="ProcessSheetFormData.processSheetNo"
                                            readonly
                                        />
                                ...
                                <b-tab active>
                                    <template #title>
                                        <feather-icon
                                            ...
                                            class="mr-0 mr-sm-50"
                                        />
                                        <span class="d-none d-sm-inline">Process Sheet</span>
                                    </template>
                                    <process-sheet-details
                                        :processSheetItemId="ProcessSheetFormData.itemId"
                                    />
                                </b-tab>
                                <b-tab>
                                    <template #title>
                                        <feather-icon
                                            ...
                                            class="mr-0 mr-sm-50"
                                        />
                                        <span class="d-none d-sm-inline">Scrap Detail</span>
                                    </template>
                                    <scrap-details
                                        :ps-scrap-tab-data.sync="ProcessSheetFormData.scrapDetail"
                                        :ps-item-list-data.sync="ItemListOption"
                                    />
                                </b-tab>
                                <b-tab>
                                    <template #title>
                                    ...
                                    <login-tab-details
                                        :ps-login-details.sync="ProcessSheetFormData.loginDetail"
                                    />
                                </b-tab>
                            </b-tabs>
                        </b-col>
...
export default {
    ...
    setup() {
        ...
        store.dispatch('ps-module/fetchProcessSheetMasterDetail', {id: router.currentRoute.params.id})
            .then(response => {
                ProcessSheetFormData.value = response.data.data
                ProcessSheetFormData.value.id = response.data.data._id
                delete ProcessSheetFormData.value._id
                showOverlayLoading.value = false
            })
            .catch(error => {
                if (error.response.status === 404) {
                    ProcessSheetFormData.value = undefined
                    showOverlayLoading.value = false
                }
            })
                ...
                if (success) {
                    axios.post('/api/new/job-card-process-sheet-item-code-exist',
                        ...
                        if (res.data.data === true) {
                            ...
                            this.ProcessSheetFormData.loginDetail.lastModifiedBy = this.CurrentUserData.first_name + ' ' + this.CurrentUserData.last_name
                            this.ProcessSheetFormData.loginDetail.modifiedDateTime = moment(new Date()).format('DD/MM/YYYY HH:mm')
                            store.dispatch('ps-module/updateProcessSheetMaster', this.ProcessSheetFormData).then(response => {
                                if (response.status === 200) {
                                    // this.$router.push({name: 'process-sheet-master'})
                                    this.$toast({
                                        component: ToastificationContent,
                                        position: 'top-right',
                                        props: {
                                            title: 'Process Sheet Edited',
                                            icon: 'CheckSquareIcon',
                                            variant: 'success',
                                            text: `You have successfully Edited Process Sheet ${this.ProcessSheetFormData.processSheetNo}.`,
                                        },
                                    })
                                }
                            })
