    ...
    export default {
        ...
        setup() {
            ...
            const refetchData = async () => {
                ...
                await axios.get(`/api/new/job-work-outside-qc/${router.currentRoute.params.id}`).then((res) => {
            ...
            const saveFormData = async () => {
                await store.dispatch('quality-control-store-module/updateJobWorkOutsideQc', formData.value).then(res => {
                    if (res.status === 200) {
                        toast({
                            component: ToastificationContent,
                            position: 'top-right',
                            props: {
                                title: 'Job Work Outside Qc Updated',
                                icon: 'CheckSquareIcon',
                                variant: 'success',
                                text: 'You have successfully Updated Job Work Outside Qc',
                            },
                        })
                    } else {
                        toast({
