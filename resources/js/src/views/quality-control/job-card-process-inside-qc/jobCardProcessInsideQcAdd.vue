<template>
    <div>
                        ...
                        <b-col md="6">
                                    ...
                                    >
                                        ...
                                        >
                                            <b-input-group>
                                                ...
                                                <b-input-group-append>
                                                    <b-form-datepicker
                                                        v-model="formatDates.receiptDate"
                                                        button-only
                                                        @input="getReceiptFormatDate($event)"
                                                        show-decade-nav
                                                        size="sm"
                                                        nav-button-variant="primary"
                                                    />
                            ...
                            <b-row>
                                <b-col md="6">
                                    <validation-provider
                                        #default="validationContext"
                                        name="Pending Inspection Qty"
                                        rules=""
                                    >
                                        <b-form-group
                                            label="Pending Inspection Qty"
                                            label-for="pendingInspectonQty"
                                        >
                                            <b-form-input
                                                id="pendingInspectonQty"
                                                v-model="formData.pendingInspectonQty"
                                                :state="getValidationState(validationContext)"
                                            />
                                            <b-form-invalid-feedback :state="getValidationState(validationContext)">
                                                {{ validationContext.errors[0] }}
                                ...
                                <b-col md="6">
                                    ...
                                    >
                                        ...
                                        >
                                            <b-form-input
                                                id="okQty"
                                                v-model="formData.okQty"
                                                :state="getValidationState(validationContext)"
                                                @keyup="addAcceptedQty"
                                            />
                                            <b-form-invalid-feedback :state="getValidationState(validationContext)">
                                                {{ validationContext.errors[0] }}
                                            </b-form-invalid-feedback>
        ...
        <!--Modal in Job Card item-->
        <b-modal
            id="jobCardItemModal"
            title="Select Item Detail"
            ref="jobCardItemModal"
            ok-title="submit"
        ...
        >
            ...
            <b-table
                responsive
                :fields="modalTableData.jobCardItemFields"
                :items="modalTableData.jobCardItemData"
                bordered
                hover
                ref="table"
                show-empty
            >
                <template #cell(selectItem)="data">
                    <b-button
                        ...
                        type="button"
                        size="sm"
                        @click="selectJobCardItemItem(data.item)"
                    >
                        Select
                    </b-button>
                </template>
...
<script>
    ...
    export default {
        ...
        setup({jobCardItemModal}) {
            ...
            axios.get('/api/new/get-job-card-item-options').then(res => {
            ...
            const modalTableData = ref({
                jobCardItemFields: [
                    ...
                    {
                        key: 'process_details.processDescription',
                        label: 'Process'
                    },
                    // {
                    //     key: 'operator_name.InSideOperatorName',
                    //     label: 'Operator Name'
                    // },
                ],
                jobCardItemData: [],
            ...
            const getJobFormatDate = (date) => {
                formData.value.jobCardDate = forMatIndianDate(date)
            }
            const getReceiptFormatDate = (date) => {
                formData.value.receiptDate = forMatIndianDate(date)
            }
            ...
            const blankFormData = {
                ...
                jobCardDate:null,
                workOrderNo:null,
                receiptDate:forMatIndianDate(getTcTodayDate()),
                itemStore:null,
                pendingInspectonQty:null,
                okQty:null,
                acceptedQty:null,
                rejectedQty:null,
                reWorkQty:null,
            ...
            const openModalItem = () => {
                jobCardItemModal.value.show();
            }

            const selectJobCardItemItem = (data) => {
                ...
                formData.value.itemDrawingNo = data?.drawingNumber ?? null
                formData.value.itemRev = data?.drawingRevesion ?? null
                formData.value.itemUom = data?.uom ?? null
                formData.value.jobCardNo = data?.jobCardNumber ?? null
                formData.value.jobCardDate = data?.jobCardDate ?? null
                formData.value.workOrderNo = data?.woNumber ?? null
                formData.value.acceptedQty = 0
                formData.value.rejectedQty = 0
                formData.value.okQty = 0
                formData.value.reWorkQty = 0
                formData.value.pendingInspectonQty = data?.pendingJobQty ?? null
                jobCardItemModal.value.hide();
            }
            ...
            const saveFormData = async () => {
                await store.dispatch('quality-control-store-module/addJobCardProcessInsideQc', formData.value).then(res => {
                    if (res.status === 200) {
                        toast({
                            component: ToastificationContent,
                            position: 'top-right',
                            props: {
                                title: 'Job Card Process Inside QC Added',
                                icon: 'CheckSquareIcon',
                                variant: 'success',
                                text: `You have successfully Added Job Card Process Inside QC.`,
                            },
                        })
                    } else {
                        toast({
            ...
            const addAcceptedQty = () => {
                formData.value.acceptedQty = formData.value.okQty
                formData.value.pendingInspectonQty = formData.value.pendingInspectonQty - formData.value.okQty
            }

            const getAcceptedValue = () => {
                formData.value.acceptedQty = parseInt(formData.value.okQty) - parseInt(formData.value.rejectedQty)
            }
            ...
            return {
                ...
                jobCardItemModal,
                isModalTableBusy,
                getJobCardItemFilter,
                searchData,
                modalTableData,
                selectJobCardItemItem,
                itemListOptions,
                getItemDetails,
