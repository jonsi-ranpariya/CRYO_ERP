                    ...
                    <b-tab active>
                        <template #title>
                            <feather-icon
                                ...
                                class="mr-0 mr-sm-50"
                            />
                            <span class="d-none d-sm-inline">WO Allocation</span>
                        </template>
                        <hr>
                        <wo-allocation-tab
                    ...
                    <b-tab>
                        <template #title>
                            <feather-icon
                                ...
                                class="mr-0 mr-sm-50"
                            />
                            <span class="d-none d-sm-inline">Delivery Schedule</span>
                        </template>
                        <hr>
...
import WoAllocationTab from "./tabs/woAllocationTab";
...
export default {
    ...
    setup() {
        ...
        const saveFormData = async () => {
            ...
            await store.dispatch('mrp-store-module/updateMinimumLevelShortage', formData.value).then(res => {
                if (res.status === 200) {
                    toast({
                        component: ToastificationContent,
                        position: 'top-right',
                        props: {
                            title: 'Minimum Level Shortage Updated',
                            icon: 'CheckSquareIcon',
                            variant: 'success',
                            text: `You have successfully Updated Minimum Level Shortage.`,
                        },
                    })
                } else {
                    toast({
        ...
        const refetchData = async (id) => {
            ...
            await axios.get(`/api/mrp/manual-planning/${id}`).then(res => {
                formData.value = res.data.data
                formData.value.itemMsnNumber = res.data.data?.itemMsnNumber ?? null
                formData.value.itemHsnNumber = res.data.data?.itemHsnNumber ?? null
            });
