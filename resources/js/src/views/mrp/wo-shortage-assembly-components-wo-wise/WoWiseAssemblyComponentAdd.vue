    ...
    export default {
        ...
        setup({workOrderModal}) {
            ...
            const blankFormData = {
                woNumber: null,
                woIds: [],
                indentCreatedFrom: 'woWiseAssemblyComponent',
                indentCreatedMenuId: 6,
            }
            ...
            const selectModalWorkOrder = async (data) => {
                ...
                await axios.get(`/api/get-work-order-assembly-items/${data._id}`).then(res => {
            ...
            const saveFormData = async () => {
                if (formData.value.woIds.length === 0) {
                    ...
                    await store.dispatch('mrp-store-module/addWoWiseAssemblyComponent', formData.value).then(res => {
                        if (res.status === 200) {
                            toast({
                                component: ToastificationContent,
                                position: 'top-right',
                                props: {
                                    title: 'WO Shortage Assembly Component (Wo Wise) Added',
                                    icon: 'CheckSquareIcon',
                                    variant: 'success',
                                    text: `You have successfully Added WO Shortage Assembly Component (Wo Wise).`,
                                },
                            })
                        } else {
                            toast({
